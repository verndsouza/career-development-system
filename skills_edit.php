...
if ($_SERVER["REQUEST_METHOD"] == "POST"|| isset($_POST["submit"]))
{
    ...
    $conn->query($sql);
        ...
        $sql = "UPDATE skill_set SET skill = '$skill', prof = '$prof', certi = '$cert' WHERE user_id = '$a'";
        $conn->query($sql);
...
if ($_SERVER["REQUEST_METHOD"] == "GET" || $i == 1)
{
	echo'<section>
		...
		$sql = "SELECT skill FROM all_skills
		  WHERE role_tag =
		  (SELECT role_1_tag FROM desired_role
		    WHERE user_id = '$a')";
		...
		<div>
    <button class="btn btn-defaul" type="submit" name="submit" value="submit"/>Update<br/></div></form>
		</section>';
}
