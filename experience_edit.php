...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["submit"]))
{
  ...
  if($i == 0)
  {
    ...
    $sql = "UPDATE experience SET level = '$level', role_2 = '$erole', role_2_tag = '$role_tag', start_date = '$spanStart', end_date = '$spanEnd', years = '$span', company = '$comp', location = '$location' WHERE user_id = '$a'";
    $conn->query($sql);
...
if ($_SERVER["REQUEST_METHOD"] == "GET" || $i == 1)
{
  echo'<section>
    ...
    IN (SELECT major_tag FROM education WHERE user_id = '$a')";
    $result = $conn->query($sql);
